  ...
  <div class="app-page">
    <div class="page-content stack">
      <header class="page-header sticky-header">
        ...
        <button class="btn-primary save-button" type="submit" form="entry-form" :disabled="saving"><AppIcon name="check" :size="18" /><span class="save-button__label">{{ saving ? '保存中…' : '保存' }}</span></button>
      </header>
...
<style scoped>
.sticky-header { position: sticky; top: var(--safe-top); z-index: 20; padding: 13px 0; background: color-mix(in srgb, var(--color-bg) 90%, transparent); backdrop-filter: blur(18px); }.save-button { position: relative; min-width: 110px; padding-inline: 38px; }.save-button > .app-icon { position: absolute; left: 14px; }.save-button__label { display: block; width: 100%; text-align: center; white-space: nowrap; }
...
.favorite-switch, .compact-switch { min-height: var(--control-height); display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 8px 13px; border: 1px solid var(--color-border-strong); border-radius: var(--radius-sm); background: var(--color-bg-soft); cursor: pointer; }.favorite-switch span { display: grid; gap: 2px; }.favorite-switch strong { font-size: 12px; }.favorite-switch small { color: var(--color-text-muted); font-size: 12px; }
.favorite-switch input, .compact-switch input { appearance: none; position: relative; flex: 0 0 auto; width: 46px; height: 27px; margin: 0; border: 1px solid var(--color-border-strong); border-radius: 99px; background: var(--color-bg-subtle); cursor: pointer; transition: background var(--transition-fast), border-color var(--transition-fast); }
.favorite-switch input::after, .compact-switch input::after { content: ''; position: absolute; top: 2px; left: 2px; width: 21px; height: 21px; border-radius: 50%; background: var(--color-surface-elevated); box-shadow: 0 1px 3px rgba(0, 0, 0, .28); transition: transform var(--transition-fast); }
.favorite-switch input:checked, .compact-switch input:checked { border-color: var(--color-primary); background: var(--color-primary); }
.favorite-switch input:checked::after, .compact-switch input:checked::after { transform: translateX(19px); background: var(--color-on-primary); }
...
.nested-list { display: grid; gap: 12px; }.nested-card { display: grid; gap: 14px; padding: 16px; border: 1px solid var(--color-border); border-radius: 16px; background: var(--color-bg-soft); }.nested-card__header { display: flex; align-items: center; gap: 10px; }.nested-index { color: var(--color-text-muted); font-size: 12px; }.nested-card__header strong { font-size: 13px; }.nested-actions { display: flex; gap: 5px; }.nested-actions .btn-icon { width: 44px; min-width: 44px; min-height: 44px; }.danger-icon { color: var(--color-danger); }.compact-switch { justify-self: start; min-height: 44px; padding-block: 5px; color: var(--color-text-secondary); font-size: 12px; }.compact-switch span { display: flex; align-items: center; gap: 7px; }.totp-mark { width: 44px; height: 44px; display: grid; place-items: center; border-radius: 11px; background: color-mix(in srgb, var(--color-success) 11%, transparent); color: var(--color-success); }

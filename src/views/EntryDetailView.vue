...
function formatTime(value: number | undefined): string {
  return value === undefined ? '从未使用' : new Date(value).toLocaleString('zh-CN')
}
        ...
        <div class="detail-layout">
          <main class="detail-main stack">
            <section class="card detail-card">
              ...
              <div class="info-row"><span class="info-row__icon"><AppIcon name="globe" :size="18" /></span><span class="info-row__copy"><small>网址</small><strong :class="{ 'value-empty': !entry.url }">{{ entry.url || '未填写' }}</strong></span><button v-if="entry.url" class="row-action" type="button" @click="openUrl"><AppIcon name="globe" :size="17" />打开</button></div>
              <div class="info-row"><span class="info-row__icon"><AppIcon name="user" :size="18" /></span><span class="info-row__copy"><small>账号</small><strong :class="{ 'value-empty': !entry.username }">{{ entry.username || '未填写' }}</strong></span><button v-if="entry.username" class="row-action" type="button" @click="copy(entry.username, '账号')"><AppIcon name="copy" :size="17" />复制</button></div>
              <div class="info-row"><span class="info-row__icon"><AppIcon name="lock" :size="18" /></span><span class="info-row__copy"><small>密码</small><strong class="mono password-mask" :class="{ 'value-empty': !entry.password }">{{ entry.password ? '••••••••••••' : '未填写' }}</strong></span><button v-if="entry.password" class="row-action" type="button" @click="copy(entry.password, '密码')"><AppIcon name="copy" :size="17" />复制</button></div>
            ...
            <section v-if="entry.customFields.length" class="card detail-card">
              ...
              <div v-for="field in entry.customFields" :key="field.id" class="info-row"><span class="info-row__icon"><AppIcon :name="field.masked ? 'lock' : 'file'" :size="18" /></span><span class="info-row__copy"><small>{{ field.name }}</small><strong class="mono" :class="{ 'value-empty': !field.value }">{{ field.masked && !revealedFields.has(field.id) ? '••••••••' : field.value || '未填写' }}</strong></span><button v-if="field.masked" class="row-action row-action--icon" type="button" :aria-label="revealedFields.has(field.id) ? '隐藏字段' : '显示字段'" @click="toggleField(field.id)"><AppIcon :name="revealedFields.has(field.id) ? 'eyeOff' : 'eye'" :size="17" /></button><button v-if="field.value" class="row-action" type="button" @click="copy(field.value, field.name)"><AppIcon name="copy" :size="17" />复制</button></div>
            </section>
